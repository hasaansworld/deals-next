	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			...
			for (const key in values) {
				if (['iconFile', 'image1', 'image2', 'image3'].includes(key) && values[key]) {
					const compressed = await compress(values[key], key === 'iconFile' ? 150 : 900);
						...
						{user ? (
							<>
								...
								<form className="mt-14 flex w-full flex-col" onSubmit={formik.handleSubmit}>
									...
									<p className={`text-sm ${formik.submitCount > 0 && formik.errors.image1 ? 'text-rose-500' : 'text-neutral-400'}`}>
										At least 1 required{' '}
									</p>
									...
									<div className="mt-6 flex items-start gap-6">
										<div className="flex-1">
											...
											<div className="relative w-full">
												<Input
													...
													className={`mt-1 pl-20 text-center text-base ${
														formik.submitCount > 0 && formik.errors.price ? 'border-rose-500' : ''
													}`}
												/>
											...
											{formik.submitCount > 0 && formik.errors.price && (
												<p className="m-0 text-sm font-medium text-rose-500">{formik.errors.price}</p>
											)}
										</div>
										<div className="flex-1">
									...
									>
										{isSubmitting && (
											<Oval width="18" height="18" strokeWidth={10} color="#fff" secondaryColor="rgba(255, 255, 255, 0.3)" />
										)}
